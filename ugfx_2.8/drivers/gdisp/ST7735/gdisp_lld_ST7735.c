...
static void set_viewport(GDisplay *g) {
	...
	write_data (g, ((g->p.x+ST7735_COL_SHIFT) >> 8) & 0xFF); 
	write_data (g, (g->p.x+ST7735_COL_SHIFT) & 0xFF); 
	write_data (g, ((g->p.x+g->p.cx-1+ST7735_COL_SHIFT) >> 8) & 0xFF);
	write_data (g, (g->p.x+g->p.cx-1+ST7735_COL_SHIFT) & 0xFF);
	write_cmd (g, ST7735_RASET);
	write_data (g, ((g->p.y+ST7735_ROW_SHIFT) >> 8) & 0xFF);
	write_data (g, (g->p.y+ST7735_ROW_SHIFT) & 0xFF);
	write_data (g, ((g->p.y+g->p.cy-1+ST7735_ROW_SHIFT) >> 8) &0xFF);
	write_data (g, (g->p.y+g->p.cy-1+ST7735_ROW_SHIFT) & 0xFF);
